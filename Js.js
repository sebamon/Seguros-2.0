...
function MostrarContacto()
{
    ...
    if(contacto==null)
    {
        ...
        {
            contacto=document.getElementById('txtEmail');
        }
        if(label.innerText=='Telefono:')
        {
            contacto=document.getElementById('txtTelefono');
        }
    }
    contacto.value='';
    if (rEmail.checked)
...
function ValidarContacto()
{
    ...
    {
        campo=document.getElementById('txtTelefono');;
    }
...
function Enviar()
{
var Nombre,Contacto,Consulta,Selector,tipoContacto,tipoConsulta,mensaje;
var bNombre=false;
var vContacto=false;
var vSeletor=false;
mensaje='Por favor complete los siguientes datos: ';
Nombre=document.getElementById('txtNombre');
tipoContacto=document.getElementsByName('tipoContacto');
Consulta=document.getElementById('txaConsulta');
Selector=document.getElementById('txtSelector');
tipoConsulta=document.getElementById('cboConsulta');
...
if(tipoContacto[0].checked)
{
    Contacto=document.getElementById('txtEmail');
}
else
{
    if(tipoContacto[1].checked)
    {  
        Contacto=document.getElementById('txtTelefono');
    }    
    else
...
if(tipoConsulta.value!='Cotizacion')
{
    vSelector=ValidarSelector();
    if(vSelector!=true)
    {
        mensaje+='\nNro de la consulta'
    }
...
if((Nombre.value!='') && (vContacto==true) && (Consulta.value!='') && (vSelector==true) && (verFecha()==true))
{
...
function verFecha() //Aca solo veo que esten completos los select de fecha, los select se cargan con la fecha correcta.
{
    var anio,mes,dia;
    bandera=false;
    anio=document.getElementById(cboAnio);
    mes=document.getElementById(cboMes);
    dia=document.getElementById(cboDia);
    if((cboAnio.value!='') && (cboMes.value!='') && (cboDia.value!=''))
    {
        bandera=true;
    }
return bandera;
...
function CargarAnio()
{
    var Anio=document.getElementById('cboAnio');
    var anioactual=new Date();
    anioactual=anioactual.getFullYear();
    for(i=1900;i<=anioactual;i++)
    {
        Anio.innerHTML+='<option>'+i+'</option>;';
    }
}
function CargarMes()
{

    var mes=document.getElementById('cboMes');
    mes.innerHTML='';
    for(i=1;i<=12;i++)
    {
...
function enArray(valor,arreglo)
{
...
function CargarDia()
{
    var anio, mes, dia, treinta, treintayuno;
    anio=document.getElementById('cboAnio');
    mes=document.getElementById('cboMes');
    dia=document.getElementById('cboDia');
    treinta=[4,6,9,11];
    ...
    else
    {
        ...
        if (enArray(mes.value,treintayuno))
        {
            for(i=1;i<=31;i++)
            {
                dia.innerHTML+='<option>'+i+'</option>;';
